...
# 1. Memoria de los licores disponibles
...
# 2. Memoria de los datos ingresados para cada licor
if 'datos' not in st.session_state:
    st.session_state['datos'] = {}

with st.sidebar:
    st.header("Gestionar Licores")
    ...
    if st.button("Eliminar Licor"):
        if len(st.session_state['licores']) > 1:
            ...
            # Si eliminamos el licor, también borramos sus datos del baúl
            if licor_a_eliminar in st.session_state['datos']:
                del st.session_state['datos'][licor_a_eliminar]
            st.rerun()
        else:
            st.error("Debe quedar al menos 1 licor en la lista.")
...
# Asegurarnos de que el licor seleccionado tenga un espacio en el "baúl"
...
# Extraemos los datos actuales del baúl para mostrarlos en pantalla
...
# --- INTERFAZ EN BLOQUES (Tarjetas visuales) ---
col_izq, col_der = st.columns(2)

# -- COLUMNA IZQUIERDA (Entradas y Salidas Base) --
with col_izq:
...
# -- COLUMNA DERECHA (Salidas/Despacho) --
with col_der:
    with st.container(border=True):
...
oz_venta = d['vt'] * config['oz_trago']
oz_cortesia = d['ct'] * config['oz_trago']
oz_tickets = d['tt'] * config['oz_trago']
total_oz_salida_tragos = oz_venta + oz_cortesia + oz_tickets
